...
export class WompiService implements IWompiService {
  ...
  public logToFile(data: any): void {
    try {
...
`;
  ...
  async createTransaction(payload: {
    ...
    try {
      // Ensure amount_in_cents is a valid number
      if (typeof payload.amount_in_cents !== 'number') {
        payload.amount_in_cents = Number(payload.amount_in_cents);
        if (isNaN(payload.amount_in_cents)) {
          throw new Error('Invalid amount_in_cents: must be a number');
        }
      }
  ...
  async tokenizeCard(cardDetails: CardDetails): Promise<any> {
    try {
      const response = await axios.post(
      ...
      this.logToFile({
        action: 'Error Tokenizing Card',
        url: `${this.apiUrl}/tokens/cards`,
        payload: cardDetails,
        error: {
          message: error.response?.data?.error?.message || 'Error tokenizing card',
          response: error.response?.data,
          status: error.response?.status,
        },
      });
